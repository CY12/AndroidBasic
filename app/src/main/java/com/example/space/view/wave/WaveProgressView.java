...
public class WaveProgressView extends View {
    ...
    private void init(Context context,AttributeSet attrs){
        ...
        waveWidth = typedArray.getDimension(R.styleable.WaveProgressView_wave_width,dip2px(context,60));
        waveHeight = typedArray.getDimension(R.styleable.WaveProgressView_wave_height,dip2px(context,10));
    ...
    private int measureSize(int defaultSize,int measureSpec) {
        ...
        if (specMode == View.MeasureSpec.EXACTLY) {
            Log.d("Test","View.MeasureSpec.EXACTLY");
            result = specSize;
        } else if (specMode == View.MeasureSpec.AT_MOST) {
            Log.d("Test","View.MeasureSpec.AT_MOST");
            result = Math.min(result, specSize);
        }
        Log.d("Test","specSize=="+specSize+"  result=="+result);
        return result;
    }
...
//        }
        ...
        //移动到右上方，也就是p0点
        wavePath.moveTo(viewSize, percent*viewSize);
        //移动到右下方，也就是p1点
        ...
        wavePath.lineTo(-waveMovingDistance, percent*viewSize);

        //从p3开始向p0方向绘制波浪曲线
        for (int i=0;i<waveNum*2;i++){
            wavePath.rQuadTo(waveWidth/2, waveHeight-percent*9, waveWidth, 0);
            wavePath.rQuadTo(waveWidth/2, -(waveHeight-percent*9), waveWidth, 0);
        }
...
//        }
        ...
        //移动到左上方，也就是p3点
        wavePath.moveTo(0, percent*viewSize);
        //移动到左下边，也就是p2点
        wavePath.lineTo(0, viewSize);
        //移动到右下方，也就是p1点
        wavePath.lineTo(viewSize, viewSize);
        //移动到右上方，也就是p0点
        wavePath.lineTo(viewSize + waveMovingDistance, percent*viewSize);
    ...
    public class WaveProgressAnim extends Animation {
        ...
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            Log.d("Test","interpolatedTime==="+interpolatedTime);
            if(percent < progressNum / maxNum){
