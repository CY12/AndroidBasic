...
public class SlideRecyclerView extends RecyclerView {
    ...
    private void testChild(float x,float y){
        final int count = getChildCount();
        Log.e("Test","count "+count);
        for (int i = count - 1; i >= 0; i--) {
            final View child = getChildAt(i);
            Log.e("Test","child.getChildAt  "+child.toString());
            ...
            if (x >= child.getLeft() + translationX
                ...
                Log.e("Test","child.getId()  "+child.toString());
            }
        }
    }
    @Override
    public boolean onInterceptTouchEvent(MotionEvent e) {
        ...
        switch (e.getAction()){
            case MotionEvent.ACTION_DOWN:
                ...
                mFirstY = y;
                mLastX = x;
                //获取点击区域所在的itemView 在给定的点下找到最上面的视图。
                mMoveView = (ViewGroup) findChildViewUnder(x, y);
                Log.e("Test","ViewGroup "+mMoveView.toString());
                testChild(x,y);
